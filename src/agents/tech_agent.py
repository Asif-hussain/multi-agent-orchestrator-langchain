...
TECH_PROMPT_TEMPLATE = """You are an IT support specialist for the company. Use the following pieces of context from the company's IT documentation to answer the question at the end.
...
class TechAgent:
    ...
    def __init__(self, langfuse_handler: Optional[CallbackHandler] = None):
        ...
        logger.info("Tech Agent instance created")
    ...
    def load_documents(self, docs_path: str = "data/tech_docs") -> None:
        """
        ...
        if not os.path.exists(docs_path):
            raise FileNotFoundError(
                f"IT documents path '{docs_path}' does not exist. "
                "Please ensure the data directory is properly set up."
            )

        logger.info(f"Loading IT documents from {docs_path}")
        ...
        if not documents:
            raise ValueError(
                ...
                "Please add IT documentation files."
            )

        logger.info(f"Loaded {len(documents)} IT documents")
        ...
        logger.info("IT vector store created successfully")
    ...
    def create_qa_chain(self) -> None:
        ...
        logger.info("IT QA chain created successfully")
    ...
    def answer_query(self, query: str) -> dict:
        ...
        logger.info(f"Processing IT query: {query[:100]}...")
    ...
    def initialize(self, docs_path: str = "data/tech_docs") -> None:
        """
        ...
        Raises:
            ...
            ValueError: If no documents are found
        """
        logger.info("Initializing Tech Agent...")
        self.load_documents(docs_path)
        self.create_qa_chain()
        logger.info("Tech Agent initialized and ready!")
